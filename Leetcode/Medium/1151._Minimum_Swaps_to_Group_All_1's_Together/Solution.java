...
class Solution {
    ...
    public int minSwaps(int[] data) {
        ...
        for (int i = 0; i < ones; i++)
            run_swaps+=data[i];
        min_swaps = ones - run_swaps;

        for (int i = ones; i < data.length; i++){
            // drop left most element and add right most element
            run_swaps = run_swaps - (data[i-ones]) + data[i];
            min_swaps = Math.min(min_swaps, ones - run_swaps);
        }
        return  min_swaps;
    }
    ...
    public void testSolution(int[] nums, int output) {
        int result = new Solution().minSwaps(nums);
