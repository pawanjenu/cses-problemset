...
class Solution {
    ...
    private static void solve() throws IOException {
        ...
        for (int a: lights) {
            max = Math.max(max, a-prev);
            prev = a;
        }
        int[] ans = new int[n];
        int lower, higher;

        // loop through backwards, to simulate as if all elements were not inserted
        for (int i = n-1; i >= 0; i--) {
            cur_ele = arr[i];
            ans[i] = max;
            // before cur_ ele was inserted and a split was done, what was the max
            // thats the reason max calculated in i, is ans for i-1 ele
            lower = lights.lower(cur_ele);
            higher  = lights.higher(cur_ele);
            max = Math.max(max, higher - lower);
            // remove the element, as it was not present.
            lights.remove(cur_ele);
