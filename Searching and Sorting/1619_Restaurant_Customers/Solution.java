...
class Solution {
    ...
    private static void solve() throws IOException {
        int n = in.nextInt();
        TreeMap<Double, Boolean> customers = new TreeMap<>();

        for (int i = 0; i < n; i++) {
            customers.put(in.nextDouble(), true);
            customers.put(in.nextDouble(), false);
        }
        int entry = 0, ans = 0;
        for (var customer: customers.entrySet()) {
            entry += customer.getValue() ? 1: -1;
            ans = Math.max(ans, entry);
