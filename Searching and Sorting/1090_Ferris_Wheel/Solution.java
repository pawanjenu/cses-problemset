...
import java.io.*;
import java.util.*;
// https://cses.fi/problemset/result/2602994/
class Solution {
    ...
    private static void solve() {
        int n = in.nextInt();
        long max_weight = in.nextLong();
        long[] kids_weight = new long[n];
        for (int i = 0; i < n; i++)
            kids_weight[i] = in.nextLong();

        sortL(kids_weight);
        int i = 0, j = n-1, ans=0;
        while (i <= j) {
            if (kids_weight[i] + kids_weight[j] <= max_weight) {
                i++;
                j--;
            }
            else {
